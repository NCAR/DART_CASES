...
contains
  ...
  subroutine fuse_cohorts(patchptr, bc_in)  
     ...
     logical, parameter :: FUSE_debug_mod = .false.   ! This debug is over-verbose
                       ...
                       if (currentCohort%pft == nextc%pft) then              
                          ...
                          if (currentCohort%canopy_layer == nextc%canopy_layer) then 
                             ...
                             if( currentCohort%isnew.eqv.nextc%isnew ) then
                                ...
                                if ( FUSE_debug_mod .and. currentCohort%isnew ) then
...
!-------------------------------------------------------------------------------------!
  ...
  subroutine copy_cohort( currentCohort,copyc )
    ...
    if ( debug_mod ) write(fates_log(),*) 'EDcohortDyn Ia ',o%npp_acc
    if ( debug_mod ) write(fates_log(),*) 'EDcohortDyn Ib ',o%resp_acc
    ...
    if ( debug_mod ) write(fates_log(),*) 'EDCohortDyn dpstoredt ',o%dbstoredt
