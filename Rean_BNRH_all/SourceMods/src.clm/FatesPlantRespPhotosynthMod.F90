module FATESPlantRespPhotosynthMod
   ...
   ! KDR sidestep fpp replacement of DE BUG by '1'
   logical   ::  debug_mod = .false.
...
!                                 call endrun(msg=errMsg(sourcefile, __LINE__))
                     ...
                     if ( debug_mod ) write(fates_log(),*) 'EDPhoto 904 ', currentCohort%resp_m
                     if ( debug_mod ) write(fates_log(),*) 'EDPhoto 905 ', currentCohort%rdark
                     if ( debug_mod ) write(fates_log(),*) 'EDPhoto 906 ', currentCohort%livestem_mr
                     if ( debug_mod ) write(fates_log(),*) 'EDPhoto 907 ', currentCohort%livecroot_mr
                     if ( debug_mod ) write(fates_log(),*) 'EDPhoto 908 ', currentCohort%froot_mr
                     ...
                     if ( debug_mod ) write(fates_log(),*) 'EDPhoto 911 ', currentCohort%gpp_tstep
                     if ( debug_mod ) write(fates_log(),*) 'EDPhoto 912 ', currentCohort%resp_tstep
                     if ( debug_mod ) write(fates_log(),*) 'EDPhoto 913 ', currentCohort%resp_m
...
!        if ( debug_mod ) write(fates_log(),*) 'EDphot 594 ',laisun_lsl
!        if ( debug_mod ) write(fates_log(),*) 'EDphot 595 ',laisha_lsl

        !is there leaf area? - (NV can be larger than 0 with only stem area if deciduous)
        if ( laisun_lsl + laisha_lsl > 0._r8 ) then 

!           if ( debug_mod ) write(fates_log(),*) '600 in laisun, laisha loop '
...
!              if ( debug_mod ) write(fates_log(),*) 'EDPhoto 737 ', psn_out
!              if ( debug_mod ) write(fates_log(),*) 'EDPhoto 738 ', agross
!              if ( debug_mod ) write(fates_log(),*) 'EDPhoto 739 ', f_sun_lsl
...
!              if ( debug_mod ) write(fates_log(),*) 'EDPhoto 758 ', psn_out
!              if ( debug_mod ) write(fates_log(),*) 'EDPhoto 759 ', agross
!              if ( debug_mod ) write(fates_log(),*) 'EDPhoto 760 ', f_sun_lsl
