  ...
  subroutine ncd_io_char_var1_nf(varname, data, flag, ncid, readvar, nt )
    ...
    elseif (flag == 'write') then
       ...
       if (present(nt))      then
          ...
          tmpString = ' '
          do m = 1,count(1)
             ! KDR shouldn't this be tmpString(m)?  Since it's an array of single characters?
             !     I guess m:m means the same thing, but things are weird
             ! tmpString(m:m) = data(m:m)
             tmpString(m) = data(m:m)
          end do
          ! KDR pass all the ' 's to pio_put_var, as well as the file name.
          ! End of month history (not hr) file; is broken because cdate, ctime, aren't 255 chars long.
          ! len gets the length of the variable passed in, not length of the contents, which len_trim did.
          ! count(1) = 255
          count(1) = len(data)

          start(1) = 1
          start(2) = nt
          ! KDR tmpString(1:...) may be causing a crash.
...
!------------------------------------------------------------------------
  ...
  subroutine ncd_io_char_var2_nf(varname, data, flag, ncid, readvar, nt)
    ...
    if (flag == 'read') then
       ...
       if (varpresent) then
          ! KDR I think this initialization should be outside the if-block
          data   = ' '
          status = pio_get_var(ncid, varid, data)
       endif
       if (present(readvar)) readvar = varpresent
    ...
    elseif (flag == 'write') then
       ...
       if (present(nt))      then
          ...
          count(2) = len(data)
          count(3) = 1
          write(iulog,'(3A,3I5)') subname,' Before pio_put_var, varname, size, len, tapes = ', &
                trim(varname), count
          status = pio_put_var(ncid, varid, start, count, data)
       else
          status = pio_put_var(ncid, varid, data)
