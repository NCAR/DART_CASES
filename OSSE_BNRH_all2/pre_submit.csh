...
setenv RESUBMIT_SETS_CONTINUE_RUN `./xmlquery RESUBMIT_SETS_CONTINUE_RUN --value`
if ($RESUBMIT_SETS_CONTINUE_RUN == 'FALSE') then
   echo "ERROR: RESUBMIT_SETS_CONTINUE_RUN = FALSE will set CONTINUE_RUN = FALSE (despite the documentation)"
   exit 11
endif
...
# after it was manually activated for the last job of a month.
...
# 2024-5-1; python was available through a module on cheyenne, but only through conda on derecho.
# 2024-5-3; I don't know whether the system will detect whether case_run.py is 
# younger than the compiled version and automatically recompile.  Recompile to be safe.
# cheyenne: python -m py_compile case_run.py
# These MUST be run before any python commands. The first is for tcsh batch jobs.
source /etc/profile.d/z00_modules.csh >& /dev/null
module load conda
conda activate npl
# Debugging -v -v print a message for each file that is checked for when  searching  for  a  module.
#    It will show where the compiled py_compile is:
#    set line = `python -v -v -m py_compile case_run.py | grep py_compile | grep 'code object from'`
# This will create a new .pyc (in __pycache__) if case_run.py is a new link:
python -m py_compile case_run.py 
echo "case_run.pyc is in"
ls -l case_run.pyc */case_run*pyc
