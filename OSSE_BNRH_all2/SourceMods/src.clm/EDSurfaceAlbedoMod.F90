...
#include "shr_assert.h"
  ...
  logical :: debug_mod = .false.  ! for debugging this module
 ...
 subroutine ED_SunShadeFracs(nsites, sites,bc_in,bc_out)
    ...
    do s = 1,nsites
       ...
       do while (associated(cpatch))                 
          ...
          if( debug_mod ) write(fates_log(),*) 'edsurfRad_5600',ifp,s,cpatch%NCL_p,numpft
          ...
          do CL = 1, cpatch%NCL_p
             do FT = 1,numpft
                ...
                if( debug_mod ) write(fates_log(),*) 'edsurfRad_5601',CL,FT,cpatch%nrad(CL,ft)
                ...
                do iv = 1, cpatch%nrad(CL,ft) !NORMAL CASE. 
                   ...
                   if ( debug_mod ) write(fates_log(),*) 'edsurfRad 570 ',cpatch%elai_profile(CL,ft,iv)
                   if ( debug_mod ) write(fates_log(),*) 'edsurfRad 571 ',cpatch%f_sun(CL,ft,iv)
         ...
         if ( debug_mod ) write(fates_log(),*) 'edsurfRad 645 ',cpatch%NCL_p,numpft
         
         do CL = 1, cpatch%NCL_p
            do FT = 1,numpft
               
               if ( debug_mod ) write(fates_log(),*) 'edsurfRad 649 ',cpatch%nrad(CL,ft)
               ...
               do iv = 1, cpatch%nrad(CL,ft)
                  ...
                  if ( debug_mod )write(fates_log(),*) 'edsurfRad 663 ', cpatch%ed_parsun_z(CL,ft,iv)
                  ...
                  if ( debug_mod ) write(fates_log(),*) 'edsurfRad 669 ', cpatch%ed_parsha_z(CL,ft,iv)
