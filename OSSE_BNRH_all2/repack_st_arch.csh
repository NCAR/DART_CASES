See notes in ../Zagar_OSSE_TUV/repack_st_arch.csh about the differences between this and that.
This is a cleaned up version of the one in Z_OSSE_TUV_noinfl.
UNTESTED; be patient and careful.
>>> Add spaces in front of skipper lines.
...
# > > > WARNING; cheyenne compute nodes do not have direct access to Campaign Storage. < < < 
#       But gci enables transfers there.
...
if (`which mpiexec_mpt > /dev/null; echo $status` == 0) then
   ...
   # Needed (before 2022) for mpiexec_mpt:  
   setenv MPI_SHEPHERD true
else if (`which mpirun > /dev/null; echo $status` == 0) then
# debug, orig;   
   set mpi_cmd = "mpirun"
#    set mpi_cmd = "mpirun -report-pid -report-uri"
...
# Z_OSSE_TUV_noinfl; see 'skipper' below, which lets script create the cmds_template,
# but not process the nonexistent forcing files.
...
# Z_OSSE; no -00000 restarts, and I want to archive the Keep_2017-12-31-64800 restarts manually.
...
if ($do_forcing == true) then
   ...
   if ($#files_dates == 0) then
      echo "WARNING: there are no ${data_CASE}.cpl_0001.ha2x1d files.  Set do_forcing = false?"
      set skipper = 'true'
   endif
...
# Z_OSSE_TUV_noinfl; starting in Dec, so don't exit.
#             else
...
#                 " && sleep 5; rm ${files}-* &>> tar_${stage}_${stat}_$PMI_RANK.eo" \
   ...
   if ($mpi_status == 0 && $eo_failure != 0) then
      ...
      echo '      grep tar *.eo  yielded status '$eo_failure
      ls -l *.eo
      exit 70
   endif
