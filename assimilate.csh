...
# Let's also say that it's the last cycle of job - which automatically kicks off
...
#  setenv NUMTASKS_PERNODE `grep $ANY_OLD_NODE $SLURM_NODELIST | wc -l`
   ...
   # Cheyenne: setenv          LAUNCHCMD mpiexec_mpt
   # Derecho
...
set BASEOBSDIR = /glade/campaign/cisl/fs1_p_relocation/dares//Observations/Synthetic/UVTRadiosonde_Tropics
...
# Move the hidden restart set (the older one) back into $rundir 
# so that it is processed properly.
...
if ($#log_list >= 3) then
   ...
   # 2024-3-28 Added *.nc to protect against the case where a compression was interrupted
   # and there are cpl.r....nc.gz files, which confuses the counting and selection of date.
   set re_list = `${LIST} -t *cpl.r.*.nc`
   ...
   if ( $sec_o_day !~ '00000' || \
      ...
      echo "     ${CASE}"'*.{r,rs,rs1,rh[0-9]}.*'"${day_time}"
      ...
      set line = `ls -l *0001.i.*${rm_date}.nc`
      echo "sticky bit? $line"
      set sticky = `echo $line[1] | cut -c10`
      if ($sticky == 't') then
...
if (  -e ${OBS_FILE} ) then
   ...
   echo "ERROR ... no observation file ${OBS_FILE}"
   # Be sure that purging is finished, so that assim-only run won't see too many cesm.log files.
   wait
   exit 81
endif
...
# >>> Be sure to point the cam_initial_####.nc to the correct base state (truth) file
#     BEFORE running the first hindcast. <<<
...
if ($cycle == $DATA_ASSIMILATION_CYCLES) then
   ...
   if ($USING_POSTE_INFLATION == true || $USING_PRIOR_INFLATION == true) then
