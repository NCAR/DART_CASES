! This file started as cime//src/share/streams/shr_stream_mod.F90.debug
! It's included here while trying to fix the SST years errors introduced when a case was built.
! SSTICE_YEAR_{START,ALIGN,END} = 2017.
! They should be 
! SSTICE_YEAR_START = 2011
! SSTICE_YEAR_ALIGN = 2011
! SSTICE_YEAR_END   = 2020
! The bad values are read in from a binary restart file (rs1),
! so I need to change them when they are read in.
! Actually, those years are not read in explicitly.
! The restart files propagate information about which data of the original avhrr file to use.
! They have a bunch of derived indices in them ({k,n,found}_gvd, ...), which are not changed
! by changing the SSTICE namelist variables in env_run.xml.
! So I needed to create new restart files in a case that specifies the right years,
! then transplant them.
! This version has better diagnostic print statements than the original.
...
! KDR I think that found_gvd is read in from some sort of restart file (rs1?), to override the default value (false).
!     So are strm%k_gvd and n_gvd (around line 2858).
!     So it could be a problem to change the ALIGN and START and END SSTICE variables in env_run.xml
!     when there's an existing restart file.
...
! orig    
       ...
       if (limit) then
          write(s_logunit,*) trim(subName)," ERROR: limit on and rDateIn >= rDategvd",rDateIn,rDategvd
          call shr_stream_abort(trim(subName)//" ERROR: rDateIn >= rDategvd, limit true")
          return
       endif
          ...
          if (haveData) then     ! ie. if arrays have been allocated
             ...
             if (strm(k)%nFiles >= n) then
                if (trim(name) == trim(strm(k)%file(n)%name)) then
                   ! KDR This is the STREAM filename, not the docn restart file name
                   ! write(s_logunit,F05) "reading time axis for stream restart filename ",k,n, &
                   write(s_logunit,F05) "reading time axis for stream filename ",k,n, &
                   ...
                   write(s_logunit,F05) "WARNING, skip time axis for stream filename ",k,n,&
                        ' ',trim(name),' ',trim(strm(k)%file(n)%name)
                   readok = .false.
...
!     2021: fileName is attached to nUnit.  It's the rs1 restart file, 
!           which was derived from a previous hindcast.
!           If that hindcast mis-specified the SSTICE years to use, 
!           then I don't want to use these values.
...
!      What should be printed to the logunit below is n_lvd, not n.
...
!      Marvelous coding.
          write(s_logunit,"(a,2i,l,' ',a)") &
               "(shr_stream_restRead) setting {k,n,found} of new restart file for lvd for stream file ", &
               k_lvd,n_lvd, found_lvd, strm(1)%file(k_lvd)%name
!                k_lvd,n_lvd, found_lvd, strm(k_lvd)%file(n_lvd)%name
          write(s_logunit,"(a,2i,l,' ',a)") &
               "(shr_stream_restRead) setting {k,n,found} of new restart file for gvd for stream file ", &
